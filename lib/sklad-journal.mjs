...
export function logEvent({
  ...
  if (eventApiConfigured()) {
    const body = {
      ...
      source: ev.source,
      at: ev.at,
      note: ev.note || ev.what,
    };
    if (ev.type === "took") {
      postObjectTake({ who: ev.who, what: ev.what, zone: ev.zone, source: ev.source, at: ev.at, note: ev.note }).catch(
        (e) => console.warn("event-api took:", e.message)
      );
    } else {
      postPersonEvent(body).catch((e) => console.warn("event-api person:", e.message));
    }
  }
...
export async function answerJournalQuery(text, zoneHint = "") {
  if (eventApiConfigured()) {
    try {
      const r = await fetchJournalSummary(dayKey(), zoneHint);
      if (r?.text) return r.text;
    } catch (e) {
      console.warn("journal api:", e.message);
    }
  }
  return answerJournalQueryLocal(text, zoneHint);
}

function answerJournalQueryLocal(text, zoneHint = "") {
  const t = norm(text);
  const dk = dayKey();
  const zone = zoneHint || "";
