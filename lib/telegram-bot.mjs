...
import {
  confirmStrangerEnroll,
  rejectStranger,
} from "./stranger-alert.mjs";
    ...
    for (;;) {
      try {
        ...
        for (const u of updates) {
          ...
          if (cmd === "/start" || cmd === "/help") {
            await sendMessage(
              ...
              "📷 <b>QORA KO'Z</b>\n\n" +
                ...
                "/status — holat\n" +
                "/begona_ha ID Ism — begona shaxsni kiritish\n" +
                "/begona_yo ID — begona alertni rad etish\n" +
          ...
          if (cmd === "/begona_ha" || cmd === "/begona_yo") {
            if (!isAdmin(chatId) && !isMaster(chatId)) {
              await sendMessage(botToken, chatId, "⛔ Faqat admin.");
              continue;
            }
            ...
            if (!pendingId) {
              await sendMessage(
                botToken,
                chatId,
                "Foydalanish: <code>/begona_ha ID Ism Familiya</code> yoki <code>/begona_yo ID</code>"
              );
              continue;
            }
            if (cmd === "/begona_yo") {
              const out = rejectStranger(pendingId);
              await sendMessage(
                botToken,
                chatId,
                out.ok ? `✅ Begona alert rad etildi (#${pendingId.slice(0, 8)}).` : "❌ Topilmadi."
              );
              continue;
            }
            const name = parts.slice(2).join(" ").trim();
            if (!name) {
              await sendMessage(botToken, chatId, "Ism yozing: <code>/begona_ha ID Ism</code>");
              continue;
            }
            const out = confirmStrangerEnroll(pendingId, name);
            await sendMessage(
              botToken,
              chatId,
              out.ok ? `✅ ${out.message}` : `❌ ${out.error}`
            );
            continue;
          }
          ...
          if (cmd === "/status") {
            ...
            await sendMessage(
              ...
              `✅ <b>Holat</b>\n` +
                ...
                `Begona alert: ${process.env.STRANGER_ALERT_ENABLED !== "0" ? "✓" : "—"}\n` +
                `Usta: ${masterChatId() || "—"}`
            );
            continue;
          }
