...
async function replyVoice(botToken, chatId, text) {
  ...
  await sendAudio(botToken, chatId, audio, text.slice(0, 200));
}
...
async function handleBrainMessage(botToken, msg, cameraCfg, text) {
  ...
  if (
    /\b(kamer|kadr|ko'r|kur|snap|rasm|nima bor|qara)\b/u.test(lower) &&
    ezvizCloudConfigured()
  ) {
    try {
      const cam = pickCamera(cameraCfg.zones, cameraCfg.defaultId, "");
  ...
  const { reply } = await think({ text, isMasterUser: true, imageBase64: imageB64 });
