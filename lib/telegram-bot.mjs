...
import { answerArchiveQuery } from "./archive.mjs";
import {
  tryLearnFromMaster,
  wantsJournalQuery,
...
async function handleBrainMessage(botToken, msg, cameraCfg, text) {
  ...
  if (wantsAllCameras(text) && ezvizCloudConfigured()) {
    await sendMessage(botToken, chatId, "⏳ Barcha kameralardan rasm olyapman…");
    ...
    for (;;) {
      try {
        ...
        for (const u of updates) {
          ...
          if (cmd === "/start" || cmd === "/help") {
            await sendMessage(
              ...
              "📷 <b>QORA KO'Z</b>\n\n" +
                ...
                "/snap — kamera kadri\n" +
                "/snapall — barcha onlayn kameralar (20 tagacha)\n" +
          ...
          if (!text.startsWith("/")) {
            try {
              if (needsLiveCamera(text) || wantsPersonCheck(text)) {
