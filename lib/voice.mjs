...
export async function transcribeAudio(audioBuf, { mime = "audio/ogg" } = {}) {
  ...
  // Whisper API "uz" ni qabul qilmaydi — avto-tanlash + o'zbek prompt
  form.append("prompt", "O'zbekcha gapirish. Sklad, polka, kamera, mahsulot.");
...
export async function speakText(text) {
  ...
  const res = await fetch("https://api.openai.com/v1/audio/speech", {
    ...
    body: JSON.stringify({
      ...
      input,
      speed: Number(process.env.TTS_SPEED || 0.82) || 0.82,
    }),
  });
  if (!res.ok) {
